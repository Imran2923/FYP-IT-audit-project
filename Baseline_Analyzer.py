...
def windows():
    ...
    def adirectory():
        ...
        if(config['maxpage']['MaxPasswordAge'] == "42.00:00:00"):
            ...
            print("Setting 'MaxPasswordAge' requires change: Set value to equal to or more than 42.00 \n")
        ...
        if(config['minpage']['MinPasswordAge'] == "1.00:00:00"):
            ...
            print("Setting 'MinPasswordAge' requires change: Set value to equal to or more than 1.00 \n")
            
        if(int(config['minplength']['MinPasswordLength']) >= 14):
            print("No need to change Password setting: MinPasswordLength \n")
            
        else:
            print("Setting 'MinPasswordLength' requires change: Set value to equal to or more than 14 \n")
                
        if(int(config['phistorycount']['PasswordHistoryCount']) >= 24):
            print("No need to change Password setting: PasswordHistoryCount \n")
                
        else:
            print("Setting 'PasswordHistoryCount' requires change: Set value to equal to or more than 24 \n")
                    
        if(config['reverseencrypt']['ReversibleEncryptionEnabled'] == "False"):
            print("No need to change Password setting: ReversibleEncryptionEnabled \n")
                    
        else:
            print("Setting 'ReversibleEncryptionEnabled' requires change: True to False \n")
                        
        if(config['lockouttime']['LockoutDuration'] == "00:30:00"):
            print("No need to change Password setting: LockoutDuration \n")
                        
        else:
            print("Setting 'LockoutDuration' requires change: Set value to 15 or more minutes \n")
                            
        if(config['lockoutobservetime']['LockoutObservationWindow'] == "00:30:00"):
            print("No need to change Password setting: LockoutObservationWindow \n")
                            
        else:
            print("Setting 'LockoutObservationWindow' requires change: Set value to 15 or more minutes \n")
                                
        if(int(config['lockoutthreshold']['LockoutThreshold']) <= 10 and int(config['lockoutthreshold']['LockoutThreshold']) != 0 ):
            print("No need to change Password setting: LockoutThreshold \n")
                                
        else:
            print("Setting 'LockoutThreshold' requires change: Set value to 10 or fewer invalid logon attempts but not 0 \n")
